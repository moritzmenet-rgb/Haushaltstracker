...
export const ProfileSelector: React.FC<ProfileSelectorProps> = ({
  ...
  const handleCreateProfile = (e: React.FormEvent) => {
    ...
    addMember(newName.trim(), newColor, roleToAssign, 50, newPin.trim() || undefined);
    setNewName('');
    setNewPin('');
          ...
          >
            ...
            <div className="mb-8 sm:mb-12">
              ...
              <h1 className="text-3xl sm:text-4xl md:text-5xl font-black tracking-tight text-[var(--m3-on-surface)] mb-3">
                {isInitialSetup ? 'Erstelle das erste Profil' : 'Wer macht heute Haushalt?'}
              </h1>
              <p className="text-sm sm:text-base text-[var(--m3-on-surface-variant)] max-w-md mx-auto font-medium">
                {isInitialSetup
                  ? 'Starte jetzt und lege das erste Administrator-Profil für deinen Haushalt an.'
            ...
            <div className="flex flex-wrap justify-center items-center gap-6 sm:gap-10 mb-10">
              ...
              {/* Add Profile Tile (Only visible if admin or no users exist) */}
              {canAddMembers && !showAddForm && (
                ...
                >
                  ...
                  <span className="mt-3 text-sm sm:text-base font-black text-[var(--m3-on-surface-variant)] group-hover:text-[var(--m3-primary)] transition-colors">
                    {isInitialSetup ? 'Erstes Profil anlegen' : 'Profil hinzufügen'}
                  </span>
                  <span className="text-xs text-[var(--m3-primary)] font-bold">
                    {isInitialSetup ? 'Admin' : 'Nur Admin'}
                  </span>
                </motion.button>
              )}
            </div>
                    ...
                    <div className="flex items-center justify-end gap-2 pt-3 border-t border-[var(--m3-outline-variant)]/60">
                      {!isInitialSetup && (
                        <button
                          ...
                          className="px-3 py-1.5 text-xs font-bold text-[var(--m3-on-surface-variant)] hover:text-[var(--m3-on-surface)]"
                        >
                          Abbrechen
                        </button>
                      )}
                      <button
                        type="submit"
                        className="m3-btn-filled px-4 py-2 text-xs font-black"
                      >
